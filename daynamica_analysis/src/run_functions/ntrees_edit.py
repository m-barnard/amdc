...
class nTreeClus:
    #added a max clust argument -MB
    def __init__(self, sequences, n, method, ntree=10, C= None, max_clust = 6, verbose=1):
        ...
        self.max_clust                         = max_clust #setting it up here
    ...
    def finding_the_number_of_clusters(self, HC_tree_terminal_cosine, Dist_tree_terminal_cosine, which_one):
        ...
        max_clusters = min(self.max_clust + 1, len(self.sequences)) #add the +1 because of how range treates it below
        ress_sil = []
    ...
    def nTreeClus(self):
        ...
        #for all methods I am adding a self attribute for the silhoutte score in each so I can select the best method from that
        ...
        if (self.method in ["All","DT"]):
            ...
            self.running_timeDT                          = round(time.time() - start_time, 3)
            
            # MB ADD
            self.sil_tree_terminal = math.ceil(silhouette_score(squareform(self.Dist_tree_terminal_cosine),
        ...
        if (self.method in ["All","DT_position"]):
            ...
            self.running_timeDT_p                          = round(time.time() - start_time, 3)

            #MB ADD
            self.sil_tree_terminal_p = math.ceil(silhouette_score(squareform(self.Dist_tree_terminal_cosine_p),
        ...
        if (self.method in ["All","RF"]):
            ...
            self.running_timeRF                           = round(time.time() - start_time, 3)

            # MB ADD
            self.sil_RF_terminal = math.ceil(silhouette_score(squareform(self.Dist_RF_terminal_cosine),
